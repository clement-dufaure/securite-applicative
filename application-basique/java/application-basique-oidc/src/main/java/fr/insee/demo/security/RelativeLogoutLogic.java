...
/**
 * Same as Default logic except :
 * - Transform any path in parameter into relative path
 */
public class RelativeLogoutLogic extends AbstractExceptionAwareLogic implements LogoutLogic {

    public static final RelativeLogoutLogic INSTANCE = new RelativeLogoutLogic();

    private static final Logger LOGGER = LoggerFactory.getLogger(RelativeLogoutLogic.class);
    ...
    public Object perform(final WebContext context, final SessionStore sessionStore, final Config config,
        ...
        try {
            ...
            // Compute a relative url if needed
            var redirectUrl =  getAbsoluteUrl(defaultUrl,context); ;
    ...
    // general code to transform relative url into absolute

    private static int DEFAULT_HTTP_PORT = 80;
    private static int DEFAULT_HTTPS_PORT = 443;


    public static String getAbsoluteUrl(final String relativeUrl, WebContext context) {
        ...
        final var notDefaultHttpPort = WebContextHelper.isHttp(context) &&
                context.getServerPort() != DEFAULT_HTTP_PORT;
        final var notDefaultHttpsPort =WebContextHelper.isHttps(context) &&
                context.getServerPort() != DEFAULT_HTTPS_PORT;
